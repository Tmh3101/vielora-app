    ...
    <section className="relative overflow-hidden pb-20 pt-32 lg:pb-32 lg:pt-40">
      ...
      <div className="container relative z-10 mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-4xl text-center">
          ...
          >
            Vielora - Tạo chatbot AI cho website{" "}
          ...
          <motion.div
            ...
            className="mb-8 flex flex-wrap items-center justify-center gap-8 text-muted-foreground"
          >
            {[
              { icon: Zap, text: "Cài đặt trong 5 phút" },
              { icon: Shield, text: "Bảo mật dữ liệu" },
              { icon: Sparkles, text: "AI thông minh Google Gemini" },
            ].map((item, index) => (
              <motion.div
                key={item.text}
          ...
          {/* AI Summary Block (AEO Target) */}
          <motion.p
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5, delay: 0.25 }}
            className="mx-auto max-w-3xl rounded-xl border border-border/50 bg-card/30 p-4 text-sm leading-relaxed text-muted-foreground/90 backdrop-blur-sm"
          >
            <strong>Vielora</strong> là nền tảng SaaS ứng dụng kiến trúc RAG tiên tiến, giúp doanh
            nghiệp SME tại Việt Nam tự động hóa chăm sóc khách hàng. Khác với chatbot theo kịch bản
            truyền thống, Vielora biến nội dung từ website và tài liệu nội bộ thành Trợ lý AI với
            chi phí tối ưu.
          </motion.p>
        </div>
      </div>
