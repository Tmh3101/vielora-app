...
import { unpinGroupNote } from "@/lib/services/group-chat.service";
...
export async function POST(
  ...
  try {
    ...
    if (!perm.allowed || !perm.botId) {
      return NextResponse.json(
        { success: false, message: "Unauthorized: Note unpin permission required" },
    ...
    return NextResponse.json(
      { success: true, message: "Note unpinned successfully" },
      { headers: corsHeaders }
    );
  } catch (error) {
