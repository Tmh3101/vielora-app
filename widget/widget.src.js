(function (window, document) {
  ...
  /* BEGIN_WIDGET_STRINGS */
  var DEFAULT_WELCOME_MESSAGES = [
  ...
  var WIDGET_STRINGS = {
    vi: {
      ...
      creditsExceeded: "Bot đã hết credits",
      ...
      botNotReady: "Chưa sẵn sàng",
      botNotReadyDesc: "Chưa sẵn sàng",
      ...
      creditsPaused: "Tạm dừng do hết credits",
      assistantDefaultName: "Trợ lý AI",
    ...
    en: {
      ...
      creditsExceeded: "Bot is out of credits",
      ...
      botNotReadyDesc: "Not ready",
      botNotReadyWait: "{name} is not ready yet. Please wait a moment.",
      alwaysAvailable: "Always available to help",
      alwaysReady: "Always available to help",
      creditsPaused: "Paused due to insufficient credits",
      assistantDefaultName: "AI Assistant",
      stopRecording: "Stop recording",
      messageTooLong:
        "Message is too long (maximum {max} characters). Please shorten your content.",
  ...
  function getTranslation(key) {
    var lang =
      (config.settings && (config.settings.ui_language || config.settings.locale)) ||
      config.ui_language ||
      window.__vieloraLang ||
      "vi";
    var dict = WIDGET_STRINGS[lang] || WIDGET_STRINGS.vi;
    return dict[key] || (WIDGET_STRINGS.vi && WIDGET_STRINGS.vi[key]) || "";
  }

  function getInsufficientCreditsMessage() {
    return (
      state.insufficientCreditsMessage ||
      getTranslation("creditsPaused") ||
      getTranslation("creditsExceeded") ||
  ...
  function getRateLimitMessage(botName, rateLimitErrorCode, fallback) {
    switch (rateLimitErrorCode) {
      case "BOT_DAILY_LIMIT_EXCEEDED":
        return getTranslation("dailyLimitBot").replace("{name}", botName || "Bot");
      case "BOT_IP_DAILY_LIMIT_EXCEEDED":
        return getTranslation("dailyLimitUser");
      default:
        return fallback || getTranslation("dailyLimitFallback");
    }
  }
  ...
  async function initWidget(botId, options) {
    ...
    if (options && (options.lang || options.locale)) {
      config.settings.ui_language = options.lang || options.locale;
      window.__vieloraLang = options.lang || options.locale;
    }
    ...
    try {
      ...
      var rateLimitMessCreated = getRateLimitMessage(
        config.botName || "Bot",
        data.data?.errorCode,
        data.message || getTranslation("dailyLimitFallback")
      );
      ...
      if (data.success) {
        config.settings = Object.assign({}, config.settings, data.data.settings || {});
        if (data.data.ui_language) {
          config.settings.ui_language = data.data.ui_language;
          window.__vieloraLang = data.data.ui_language;
        } else if (config.settings.ui_language) {
          window.__vieloraLang = config.settings.ui_language;
        }
        state.quotaExceeded = data.data.quotaExceeded;
        state.rateLimitExceeded = !!data.data.rateLimitExceeded;
        state.rateLimitMessage = data.data?.errorCode
          ? getRateLimitMessage(
              data.data.name || "Bot",
              data.data.errorCode,
        ...
        state.subscriptionPlan =
          data.data.subscriptionPlan || data.data.settings?.subscriptionPlan || null;
        ...
        if (state.messages.length > 0) {
          ...
          setTimeout(function () {
            if (
              !state.suggestedQuestionsShown &&
              !isChatBlocked() &&
              config.settings.suggestedQuestions &&
              config.settings.suggestedQuestions.length > 0
            ) {
          ...
          setTimeout(function () {
            ...
            if (messages && state.rateLimitExceeded) {
              appendBotMessageOnce(
                state.rateLimitMessage ||
                  getRateLimitMessage(state.botName, "BOT_DAILY_LIMIT_EXCEEDED")
              );
            } else if (messages && state.isAvailable) {
              var welcomeMsg = config.settings.welcomeMessage;
              if (!welcomeMsg || DEFAULT_WELCOME_MESSAGES.indexOf(welcomeMsg.trim()) !== -1) {
                welcomeMsg = getTranslation("welcome");
              }
        ...
        var initMessage =
          response.status === 404
            ? getTranslation("unavailable") || CHATBOT_UNAVAILABLE_MESSAGE
            : data.message || getTranslation("botUnavailableFallback");
  ...
  function showWidgetInitFailure(message, openChat) {
    ...
    setTimeout(function () {
      ...
      if (input) {
        input.disabled = true;
        input.placeholder = getTranslation("botNotReadyDesc");
      }
  ...
  function getBlockedChatMessage() {
    if (state.insufficientCredits) {
      return getInsufficientCreditsMessage();
    }

    if (state.rateLimitExceeded) {
      return (
        state.rateLimitMessage || getRateLimitMessage(state.botName, "BOT_DAILY_LIMIT_EXCEEDED")
      );
    }
  ...
  function updateRateLimitBanner() {
    ...
    if (state.rateLimitExceeded || state.insufficientCredits) {
      ...
      banner.style.borderColor = state.insufficientCredits
        ? "rgba(253, 164, 175, 0.7)"
        : "rgba(253, 186, 116, 0.7)";
      banner.style.background = state.insufficientCredits
  ...
  function syncChatInputState() {
    ...
    if (input) {
      ...
      input.placeholder = state.insufficientCredits
        ? getTranslation("creditsExceeded")
        : getTranslation("typeMessage");
    ...
    if (status) {
      status.textContent = state.insufficientCredits
        ? getTranslation("creditsPaused") || getTranslation("outOfCredits")
        : state.isAvailable
          ? getTranslation("alwaysAvailable") || getTranslation("alwaysReady")
          : getTranslation("botNotReady") || getTranslation("notReady");
    }
  ...
  function getMicIconSVG(recording) {
    ...
    return (
      '<svg id="chatbotai-mic-icon" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="' +
      (config.settings.textColor || "#0f172a") +
      '" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2a3 3 0 0 0-3 3v7a3 3 0 0 0 6 0V5a3 3 0 0 0-3-3Z"></path><path d="M19 10v2a7 7 0 0 1-14 0v-2"></path><line x1="12" y1="19" x2="12" y2="22"></line></svg>'
    );
  }
    ...
    navigator.mediaDevices
      ...
      .then(function (stream) {
        ...
        if (audioTracks.length > 0) {
          ...
          var handleTrackMuteOrEnd = function () {
            appendBotMessageOnce(getTranslation("micDisconnected"));
            stopVoiceRecording();
          };
        ...
        recorder.onerror = function () {
          ...
          hideRecordingUI();
          appendBotMessageOnce(getTranslation("micDeviceError"));
          focusChatInput();
        };
      ...
      .catch(function (err) {
        ...
        if (errorName === "NotAllowedError" || errorName === "PermissionDeniedError") {
          appendBotMessageOnce(getTranslation("micPermissionRequired"));
        } else if (
          errorName === "NotReadableError" ||
          errorName === "TrackStartError" ||
          errorName === "AbortError"
        ) {
          appendBotMessageOnce(getTranslation("micInUse"));
        } else {
          appendBotMessageOnce(getTranslation("micStartFailed"));
        }
        focusChatInput();
      });
  }
  ...
  function showRecordingUI() {
    ...
    if (send) {
      send.title = getTranslation("stopRecording");
      send.disabled = false;
      send.style.opacity = "1";
      send.style.cursor = "pointer";
      send.innerHTML =
        '<svg width="14" height="14" viewBox="0 0 24 24" fill="white" stroke="white" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect></svg>';
  ...
  function sendVoiceToAPI() {
    if (state.audioChunks.length === 0) {
      appendBotMessageOnce(getTranslation("micNoAudioData"));
      focusChatInput();
      return;
    }
    ...
    if (audioBlob.size < 100) {
      appendBotMessageOnce(getTranslation("micNoAudioStream"));
      focusChatInput();
      return;
    }
    ...
    fetch(config.baseUrl + "/api/widget/voice", {
      ...
      .then(function (data) {
        ...
        if (data.success && data.text) {
          ...
          var errorMsg = data.message || getTranslation("sttFailed");
          if (data.code === "INSUFFICIENT_CREDITS") {
            state.insufficientCredits = true;
            state.insufficientCreditsMessage = data.message || getInsufficientCreditsMessage();
            errorMsg = state.insufficientCreditsMessage;
          } else if (data.code === "RATE_LIMITED") {
            errorMsg = getTranslation("sttRateLimit");
          } else if (data.code === "PLAN_RESTRICTED") {
            errorMsg = getTranslation("sttPlanRequired");
          }
      ...
      .catch(function () {
        ...
        state.isProcessingVoice = false;
        removeLastUserMessage();
        appendBotMessageOnce(getTranslation("networkError"));
        syncChatInputState();
        focusChatInput();
      });
  }
  ...
  function removeLastUserMessage() {
    ...
    if (
      lastMsg &&
      lastMsg.classList.contains("chatbotai-message-wrapper") &&
  ...
  function showWidget() {
    var existingWidget = document.getElementById("chatbotai-widget");
    if (existingWidget) existingWidget.style.display = "";
  }
  ...
  function getIconSVG(presetId) {
    var iconMap = {
      ...
      headphones:
        '<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 13.565C2 11.512 4 11 6 11v9a4 4 0 0 1-4-4zm20 0C22 11.512 20 11 18 11v9a4 4 0 0 0 4-4zM6 20V10a6 6 0 1 1 12 0v10"></path></svg>',
      ...
      "briefcase-business":
        '<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 12h.01"></path><path d="M16 6V4a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v2"></path><path d="M22 13a18.15 18.15 0 0 1-20 0"></path><rect width="20" height="14" x="2" y="6" rx="2"></rect></svg>',
      "square-arrow-out-up-left":
        '<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M13 3h6a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-6"></path><path d="m3 3 9 9"></path><path d="M3 9V3h6"></path></svg>',
      "users-round":
        '<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 21a8 8 0 0 0-16 0"></path><circle cx="10" cy="8" r="5"></circle><path d="M22 20c0-3.37-2-6.5-4-8a5 5 0 0 0-.45-8.3"></path></svg>',
      "badge-info":
      ...
      "square-user-round":
        '<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 21a6 6 0 0 0-12 0"></path><circle cx="12" cy="11" r="4"></circle><rect width="18" height="18" x="3" y="3" rx="2"></rect></svg>',
      "user-round-cog":
      ...
      "app-window":
        '<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="4" width="20" height="16" rx="2"></rect><path d="M10 4v4"></path><path d="M2 8h20"></path><path d="M6 4v4"></path></svg>',
      "hand-grab":
  ...
  function getWidgetHTML() {
    ...
    if (isLegacyPosition) {
      ...
      if (customX < viewportWidth / 2) {
        ...
        chatPositionStyle =
          "right: " + Math.round(Math.max(0, viewportWidth - customX - 64)) + "px;";
      }
    ...
    return `
      ...
      ">
        <div id="chatbotai-header" style="padding: 14px 16px 10px 16px; background-color: ${config.settings.primaryColor}; color: white; display: flex; align-items: center; gap: 12px;">
          ...
          <div style="flex: 1;">
            <div style="font-weight: 600; font-size: 16px; margin-bottom: -2px;">${state.botName || getTranslation("assistantDefaultName")}</div>
            <div id="chatbotai-status" style="font-size: 12px; opacity: 0.9; display: flex; align-items: center; gap: 6px; line-height: 1;">
              ${state.insufficientCredits ? getTranslation("creditsPaused") || getTranslation("outOfCredits") : state.isAvailable ? getTranslation("alwaysAvailable") || getTranslation("alwaysReady") : getTranslation("botNotReady") || getTranslation("notReady")}
            </div>
          </div>
        ...
        <div id="chatbotai-input-wrapper" style="display: flex; flex-direction: column; background: transparent; position: relative;">
          ...
          <div id="chatbotai-input-container" style="padding: 6px 12px; display: flex; align-items: center; gap: 8px; background: white; border-radius: 0 0 16px 16px; box-sizing: border-box;">
            <input id="chatbotai-input" type="text" placeholder="${getTranslation("typeMessage")}" ${isChatBlocked() ? "disabled" : ""} maxlength="${MAX_CHAT_INPUT}" style="
  ...
  function bindEvents() {
    ...
    if (suggestedButtons) {
      suggestedButtons.addEventListener(
        "wheel",
        function (e) {
  ...
  async function sendMessage(fromVoice) {
    ...
    if (message.length > MAX_CHAT_INPUT) {
      appendBotMessageOnce(getTranslation("messageTooLong").replace("{max}", MAX_CHAT_INPUT));
      return;
    }

    if (state.quotaExceeded) {
      appendBotMessageOnce(getTranslation("maintenance"));
      return;
    }
    if (state.insufficientCredits) {
      input.value = "";
      appendBotMessageOnce(getInsufficientCreditsMessage());
      return;
    }
    if (state.rateLimitExceeded) {
      input.value = "";
    ...
    if (!state.isAvailable) {
      appendBotMessageOnce(
        state.statusMessage ||
          getTranslation("botNotReadyWait").replace("{name}", state.botName || "Bot")
      );
      return;
    }
    ...
    try {
      ...
      if (response.status === 429) {
        state.rateLimitExceeded = true;
        state.rateLimitMessage = getRateLimitMessage(
          config.botName || "Bot",
          data.code,
          data.message || getTranslation("dailyLimitFallback")
        );
      ...
      if (response.status === 403) {
        appendBotMessageOnce(getTranslation("domainAuthError"));
        state.isLoading = false;
        syncChatInputState();
        return;
      }
      ...
      if (data.success) {
        ...
        var chatMessage =
          response.status === 404
            ? getTranslation("unavailable") || CHATBOT_UNAVAILABLE_MESSAGE
            : data.message || getTranslation("error");
        if (response.status === 404) {
          state.isAvailable = false;
  ...
  function navigateToPage(url, anchor, explicit) {
    ...
    console.log("[SmartHomepage] navigateToPage called", {
      url: url,
      anchor: anchor || null,
      explicit: explicit,
    });
    ...
    try {
      localStorage.setItem("vielora_pending_nav_" + config.botId, "1");
    } catch (e) {
      /* ignore */
    }
  ...
  function showNavigationBanner(finalUrl) {
    ...
    banner.style.cssText =
      "position:fixed;bottom:90px;left:50%;transform:translateX(-50%);background:#1e293b;color:#fff;padding:10px 14px;border-radius:10px;font-size:13px;z-index:9999;display:flex;align-items:center;gap:10px;box-shadow:0 4px 12px rgba(0,0,0,.2);";

    var text = document.createElement("span");
    text.textContent = getTranslation("redirecting");
    ...
    cancelBtn.setAttribute("aria-label", getTranslation("cancelRedirect")); // T-D
    cancelBtn.style.cssText =
      "background:#334155;color:#fff;border:none;padding:4px 10px;border-radius:6px;cursor:pointer;";
  ...
  function showLeadForm(originalQuestion) {
    ...
    inner.style.cssText =
      "background: #f1f5f9; padding: 14px; border-radius: 16px; border-bottom-left-radius: 4px; font-size: 13px; position: relative;";

    var closeBtn = document.createElement("button");
    closeBtn.innerHTML =
      '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>';
    closeBtn.title = getTranslation("close");
    closeBtn.style.cssText =
      "position: absolute; right: 6px; top: 6px; width: 24px; height: 24px; display: flex; align-items: center; justify-content: center; border: none; border-radius: 50%; background: transparent; color: #94a3b8; cursor: pointer; transition: background 0.2s, color 0.2s;";
    ...
    label.textContent = getTranslation("leadFormPrompt") || getTranslation("leadFormTitle");
    inner.appendChild(label);

    var questionP = document.createElement("p");
    questionP.style.cssText =
      "font-style: italic; color: #64748b; margin: 0 0 10px 0; font-size: 12px;";
    questionP.textContent =
      '"' +
      (originalQuestion.length > 80 ? originalQuestion.slice(0, 80) + "..." : originalQuestion) +
      '"';
    inner.appendChild(questionP);

    var fields = [
      {
        id: "chatbotai-lead-name",
        type: "text",
        placeholder: getTranslation("leadFormName") + " *",
        required: true,
        max: 100,
      },
      {
        id: "chatbotai-lead-email",
        type: "email",
        placeholder: getTranslation("leadFormEmail") + " *",
        required: true,
        max: 200,
      },
      {
        id: "chatbotai-lead-phone",
        type: "tel",
        placeholder: getTranslation("leadFormPhone"),
        required: false,
        max: 15,
      },
    ];
    ...
    fields.forEach(function (field) {
      ...
      input.style.cssText =
        "display: block; width: 100%; margin-bottom: 6px; padding: 8px 12px; border: 1px solid #e2e8f0; border-radius: 12px; font-size: 13px; outline: none; box-sizing: border-box; transition: border-color 0.2s;";
    ...
    noteTextarea.placeholder =
      getTranslation("leadFormNotePlaceholder") || getTranslation("leadFormMessage");
    noteTextarea.maxLength = 500;
    noteTextarea.style.cssText =
      "display: block; width: 100%; margin-bottom: 6px; padding: 8px 12px; border: 1px solid #e2e8f0; border-radius: 12px; font-size: 13px; outline: none; box-sizing: border-box; resize: none; transition: border-color 0.2s; font-family: inherit;";
    noteTextarea.onfocus = function () {
      this.style.borderColor = config.settings.primaryColor;
    };
    noteTextarea.onblur = function () {
      this.style.borderColor = "#e2e8f0";
    };
    noteTextarea.rows = 2;
    ...
    submitBtn.textContent = getTranslation("leadFormSubmit");
    submitBtn.style.cssText =
      "width: 100%; padding: 8px 16px; border: none; border-radius: 12px; background-color: " +
      config.settings.primaryColor +
      "; color: " +
      getUserMessageTextColor(config.settings.primaryColor) +
      "; font-size: 13px; font-weight: 500; cursor: pointer; transition: opacity 0.2s; margin-top: 4px;";
    ...
    submitBtn.onclick = function () {
      ...
      if (!name || name.length < 2) {
        showLeadError(getTranslation("leadNameMin"));
        return;
      }
      if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
        showLeadError(getTranslation("leadEmailInvalid"));
        return;
      }

      submitBtn.disabled = true;
      submitBtn.textContent = getTranslation("leadFormSubmitting");
      ...
      fetch(config.baseUrl + "/api/widget/lead", {
        ...
        .then(function (result) {
          if (result.success) {
            ...
            successDiv.style.cssText =
              "background: #f1f5f9; padding: 14px; border-radius: 16px; border-bottom-left-radius: 4px; font-size: 13px;";
            var successMsg = getTranslation("leadFormSuccess");
            successDiv.innerHTML =
              '<div style="display: flex; align-items: center; gap: 8px; color: #16a34a; font-weight: 600; margin-bottom: 4px;"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="20 6 9 17 4 12"></polyline></svg>' +
              successMsg +
              '</div><p style="margin: 0; font-size: 12px; color: #64748b;">' +
              successMsg +
              "</p>";
            ...
            showLeadError(result.message || getTranslation("error"));
        ...
        .catch(function () {
          showLeadError(getTranslation("networkError"));
          submitBtn.disabled = false;
          submitBtn.textContent = getTranslation("leadFormSubmit");
          submitBtn.style.opacity = "1";
        });
      ...
      if (isProcessing) {
        ...
        if (isVoice) {
          ...
          micBadge.innerHTML =
            ...
            badgeColor +
            '" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2a3 3 0 0 0-3 3v7a3 3 0 0 0 6 0V5a3 3 0 0 0-3-3Z"/><path d="M19 10v2a7 7 0 0 1-14 0v-2"/><line x1="12" y1="19" x2="12" y2="22"/></svg>';
          micBadge.style.cssText =
            "position: absolute; top: -7px; right: -7px; background: #ffffff; border: 1px solid #e2e8f0; border-radius: 50%; width: 22px; height: 22px; display: flex; align-items: center; justify-content: center; box-shadow: 0 2px 5px rgba(0,0,0,0.1);";
  ...
  function addSuggestedQuestions(questions) {
    ...
    questions.forEach(function (question, index) {
      ...
      btn.style.cssText =
        "display: inline-flex !important; align-items: center !important; justify-content: center !important; flex-shrink: 0; white-space: nowrap; padding: 4px 10px; height: 28px; min-height: 28px; border: 1px solid " +
        UI.inputBorder +
        "; border-radius: 9999px; background: white; box-shadow: 0 2px 8px rgba(15, 23, 42, 0.08); cursor: pointer; font-size: 12px; font-weight: 500; color: #334155; transition: all 0.2s; opacity: 0.98; line-height: 1 !important; box-sizing: border-box;";
  ...
  function parseMarkdown(text) {
    ...
    result = result.replace(
      ...
      '<a href="$2" target="_blank" rel="noopener noreferrer" style="color: ' +
        linkColor +
        '; text-decoration: underline; text-underline-offset: 2px; font-weight: 600;">$1</a>'
    );
    ...
    function flushList() {
      ...
      blocks.push(
        '<ul style="list-style-type: disc; list-style-position: outside; margin: 6px 0; padding-left: 20px;">' +
          listItems.join("") +
          "</ul>"
      );
      listItems = [];
  ...
  function loadPreviousMessages() {
    ...
    separator.style.cssText =
      "text-align: center; color: #9ca3af; font-size: 11px; padding: 10px 0;";
    separator.textContent = getTranslation("chatHistory");
  ...
  Vielora.show = showWidget;
  Vielora.hide = hideWidget;
  Vielora.reset = resetWidget;
  Vielora.remove = removeWidget;
  ...
  function autoInit() {
    ...
    if (currentScript) {
      ...
      var lang =
        currentScript.getAttribute("data-lang") || currentScript.getAttribute("data-locale");

      if (lang) {
        window.__vieloraLang = lang;
      }

      if (botId) {
        // Tái sử dụng hàm initWidget nội bộ
        initWidget(botId, { baseUrl: baseUrl, lang: lang });
      }
    }
  }
